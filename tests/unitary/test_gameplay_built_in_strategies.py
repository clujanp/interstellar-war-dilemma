...
class TestBuiltInStrategies(TestCase):
    ...
    def test_tic_for_tac_with_last_position_success(self):
        self.mock_memories.last_position.return_value = Position.AGGRESSION
        result = BuiltInStrategies.tic_for_tac(
            self.mock_opponent, self.mock_memories)
        assert result == Position.AGGRESSION
        self.mock_memories.last_position.assert_called_once_with(
            self.mock_opponent)

    def test_tic_for_tac_without_last_position_success(self):
        self.mock_memories.last_position.return_value = None
        result = BuiltInStrategies.tic_for_tac(
            self.mock_opponent, self.mock_memories)
        assert result in [Position.COOPERATION, Position.AGGRESSION]
        self.mock_memories.last_position.assert_called_once_with(
    ...
    def test_joss_with_last_position_success(self, mock_random: MagicMock):
        mock_random.return_value = 0.90
        self.mock_memories.last_position.return_value = Position.COOPERATION
        result = BuiltInStrategies.joss(self.mock_opponent, self.mock_memories)
        assert result == self.mock_memories.last_position.return_value
        mock_random.assert_called_once_with()
        self.mock_memories.last_position.assert_called_once_with(
            self.mock_opponent)

    @patch('gameplay.strategy.random')
    def test_joss_without_last_position_taken_default_success(
    ...
    def test_joss_with_random_10_percent_success(self, mock_random: MagicMock):
        ...
        assert result == Position.AGGRESSION
        mock_random.assert_called_once_with()
        self.mock_memories.last_position.assert_not_called()
    ...
    def test_tester_first_skirmishes_success(self):
        ...
        self.mock_memories.skirmishes_count.assert_called_once_with(
            self.mock_opponent)
        self.mock_memories.first_position.assert_not_called()
        self.mock_memories.last_position.assert_not_called()
    ...
    def test_tester_more_second_skirmish_with_second_aggression_success(self):
        ...
        self.mock_memories.first_position.return_value = [
        ...
        self.mock_memories.skirmishes_count.assert_called_once_with(
            self.mock_opponent)
        self.mock_memories.first_position.assert_called_once_with(
