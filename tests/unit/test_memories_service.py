...
class TestMemoriesService(TestCase):
    ...
    def test_returns_empty_when_no_history(self):
        ...
        result = self.service.get_skirmish_history(self.civ_1.id)
        assert result == []
    ...
    def test_filters_by_opponent(self):
        ...
        result = self.service.get_skirmish_history(
            self.civ_1.id, opponent_id=self.civ_3.id)
        assert len(result) == 1
    ...
    def test_limits_with_last_n(self):
        ...
        result = self.service.get_skirmish_history(self.civ_1.id, last_n=1)
        assert len(result) == 1
        assert result[0] is s2
